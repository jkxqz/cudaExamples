...
void matrixMul(float* A_h, float* B_h, float* C_h, int height_A, int width_A, int width_B) {
    ...
    // This implementation will map only 1 thread to an entire row in the output matrix
    dim3 dimGrid(1,ceil(height_A/16.0),1); // number of blocks in a grid
    dim3 dimBlock(1,16,1); // number of theads in a block
